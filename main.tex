...
\begin{document}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			Die Aussage $A(n) \Rightarrow A(n + 1)$ muss also wahr sein. Dafür startet man bei $A(n)$ und kommt durch Termumformungen auf $A(n + 1)$.
			\footnote{Vgl. \url{https://www.logik.uni-jena.de/logikmedia/75/einfuehrung-in-das-mathematische-beweisen.pdf} (stand 2.9.25)}
	...
	Danach wird gezeigt, dass, wenn $A(n)$ wahr ist, auch $A(n + 1)$ wahr ist \\ (\emph{Induktionsschluss}). \\
		...
		\[
		\begin{aligned}
			\sum_{k=1}^{n} k &=  \frac{n(n+1)}{2}                 && \quad| \;\text{+ $(n + 1)$} \\
			\sum_{k=1}^{n} k + (n + 1) &=  \frac{n(n+1)}{2} + (n + 1) && \quad| \;\text{T} \\
			\sum_{k=1}^{n + 1} k &= \frac{n(n+1)}{2} + (n + 1)    && \quad| \;\text{mit 2 erweitern} \\
			\sum_{k=1}^{n + 1} k &= \frac{n(n+1)}{2} + \frac{2(n + 1)}{2} && \quad| \;\text{zusammenfassen} \\
			\sum_{k=1}^{n + 1} k &= \frac{n(n+1) + 2(n + 1)}{2}   && \quad| \;\text{$(n + 1)$ ausklammern} \\
			\sum_{k=1}^{n + 1} k &= \frac{(n+1)(n + 2)}{2}        && \quad| \;\text{Q.E.D.}
		\end{aligned}
		\]
	\begin{comment}
		\[
