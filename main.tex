...
\begin{document}
	...
	\end{array}
	\]
	Wie zu sehen ist, ist diese Aussage nur falsch, wenn $P$ wahr ist, und $Q$ falsch.
	...
	Auf Grund der Vielfalt der Aussagen gibt es verschiedene Beweisverfahren, die zum Teil auf die selben Aussagen angewendet werden können, oder verschiedene Arten von Aussagen beweisen können.
	...
	Der Induktionsbeweis wird verwendet, um Aussagen für alle Elemente einer bestimmten Menge zu beweisen. Bei der Menge kann es sich um verschiedene Dinge handeln. Meist handelt es sich um Zahlen, es kann aber alles mögliche sein, das sich \emph induktiv definieren lässt. Wie die Induktive Mengendefinition fuinktionert wird im nächsten Abschnitt erklärt. 
	\subsubsection{Vollständige Induktion}
	Bei der vollständigen Induktion entspricht die Menge immer den natürlichen Zahlen. Man beweist also beid er vollständigen Induktion, dass eine Aussage für alle natürlichen Zahlen gilt.
	% \cite{uni-jena}
	...
	Beim Festlegen einer \emph{induktiven Menge} folgt man zwei Schritten.
	 ...
	 \end{enumerate}
	 Häufig will man etwas für alle \emph{natürlichen Zahlen} beweisen. Diese haben folgende vier Eigenschaften, die auch als ihre Definition angesehen werden können: \\
	 \begin{enumerate}
	 	...
	 	\grqq
	 \footnote{\url{https://studyflix.de/mathematik/zahlenmengen-3260/natuerliche-zahlen} (stand 3.9.25)}
	 \end{enumerate}
	 Anhand der ersten beiden Eigenschaften kann man schon erkennen, dass sich die natürlichen Zahlen als induktive Menge definieren lassen.
	...
	Die vollständige Induktionsbeweis besteht aus zwei Schritten, dem \emph Induktionsanfang und dem Induktionsschritt.
	Wenn man die Aussage A für die Menge
	\[
		M := \{n \in \mathbb{N} \mid A(n)\}
	\]
	beweisen möchte sehen die beiden Schritte folgendermaßen aus: \\
	\begin{itemize}
		\item \textbf{Induktionsanfang:} Man zeigt, dass $A(1)$ wahr ist.
		...
		Auch dieser Schritt besteht aus zwei Teilen:
		\begin{itemize}
			\item Man nimmt man an, dass $A(n)$ wahr ist. Das ist die sogenannte \\ \emph{Induktionsvoraussetzung}.
			\item Man leitet aus der Induktionsvorraussetzung her, das $A(n + 1) $ wahr ist. Das nennt man \emph{Induktionsschluss}.  
			\footnote{Vgl. \url{https://www.logik.uni-jena.de/logikmedia/75/einfuehrung-in-das-mathematische-beweisen.pdf} (stand 2.9.25)}
		\end{itemize}
	\end{itemize}	
	...
	\]
	zur beerechnung der Summer der ersten n Zahlen aufgestellt. \footnote{Vgl. \url{https://studyflix.de/mathematik/gaussche-summenformel-2408}, (stand 3.9.25)} \\ 
	\\
	Folgende Aussage soll nun bewiesen werden:
	\[
		A(n) : 1 + 2 + \dots + n = \frac{n(n+1)}{2}, \quad \forall n  \in \mathbb{N}, n \ge 1
	\]
	Dafür werden die beiden Schritte des Induktionsbeweises ausgführt.
	...
	Zeige, dass $A(n) \Rightarrow A(n + 1)$ gilt: \\ 
	Dafür wird angenommen, dass $A(n)$ wahr ist (\emph{Induktionsvoraussetzung}):
	\[
	A(n): 1 + 2 + \dots + n = \frac{n(n+1)}{2}
	\]
	Danach wird gezeigt, dass, wenn $A(n)$ wahr ist, auch $A(n + 1)$ wahr ist \\ (\emph{Induktionsschritt}): 
	\[
	\begin{aligned}
